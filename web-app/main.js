...
/**
 * Returns the number of wins for the given player in the current session.
 * @param {Orbito.Player} player - The player to retrieve wins for (1 or 2).
 * @returns {number} The total wins for that player.
 */
    ...
    function updateScores() {
        if (scoreP1El) {
            scoreP1El.textContent = getWins(1);
        }
        if (scoreP2El) {
            scoreP2El.textContent = getWins(2);
        }
    }
    updateScores();
